...
class EnhancedVisualDustSimulator:
    ...
    def get_enhanced_dust_color(self, intensity_norm):
        ...
        # 颜色点调整为较暗的土褐色系，避免过亮
        color_points = [
            (0.05, (150, 130, 100)),   # 低浓度: 深灰褐色
            (0.2, (170, 150, 120)),    # 较低浓度: 中灰褐色
            (0.4, (190, 170, 140)),    # 中等浓度: 浅灰褐色
            (0.6, (200, 180, 150)),    # 较高浓度: 浅棕褐色
            (0.8, (210, 190, 160)),    # 高浓度: 浅棕黄色
            (1.0, (220, 200, 170))     # 最高浓度: 浅黄褐色
        ]

        for i in range(len(color_points) - 1):
            t1, rgb1 = color_points[i]
            t2, rgb2 = color_points[i + 1]
    ...
    def create_enhanced_dust_layer(self, clean_img, target_concentration_g_m2):
        ...
        for y in range(0, self.height, block_size):
            for x in range(0, self.width, block_size):
                ...
                if block_intensity > 0.03:
                    ...
                    # 降低颜色噪声范围，使灰尘颜色更稳定
                    color_noise = random.randint(-10, 10)
        ...
        # 降低亮度（从0.93调整到0.85）使整体更暗
        final_img = ImageEnhance.Brightness(final_img).enhance(0.85)
        # 稍微降低对比度（从1.1调整到1.05）使效果更自然
    ...
    def process_folder(self, input_folder, num_samples=50, min_conc=0, max_conc=60):
        ...
        output_dir = f"enhanced_gray_dust_visualization"
